...
#[no_mangle]
pub extern fn pitch_downsample(x :* mut * mut celt_sig, x_lp :* mut v16,
		len :c_int, c :c_int, arch :c_int) {
	let len = len as usize;
	let len2 = len >> 1;
	let mut x_lp = unsafe {
		slice::from_raw_parts_mut(x_lp, len2)
	};
	let x_0 = unsafe {
		slice::from_raw_parts(*x, len)
	};
	let x_1 = unsafe {
		slice::from_raw_parts(*(x.offset(1)), len)
	};
	x_lp[0] = shr32!(half32!(half32!(x_0[1]) + x_0[0]), shift);
	for i in 1 .. len2 {
		x_lp[i] = shr32!(half32!(half32!(x_0[2 * i - 1] + x_0[2 * i + 1])
			+ x_0[2 * i]), shift);
	}
	if c == 2 {
		x_lp[0] += shr32!(half32!(half32!(x_1[1]) + x_1[0]), shift);
		for i in 1 .. len2 {
			x_lp[i] += shr32!(half32!(half32!(x_1[2 * i - 1] + x_1[2 * i + 1])
				+ x_1[2 * i]), shift);
		}
	}
	let mut ac = [0.0; 5];
	_celt_autocorr(x_lp.as_ptr(), (&mut ac).as_mut_ptr(), ptr::null(),
		0, 4, len as c_int >> 1, arch);
	...
	let mut tmp = Q15ONE;
	for i in 0 .. 4 {
		tmp = mul16_16_q15!(qconst16!(0.9, 15), tmp);
		lpc[i] = mul16_16_q15!(lpc[i], tmp);
	}
	let mut lpc2 = [0.0; 5];
...
#[no_mangle]
pub extern fn pitch_search(x_lp :* const v16, y :* const v16,
		len :c_int, max_pitch :c_int, pitch :&mut c_int, arch :c_int) {
	...
	let x_lp = unsafe {
		slice::from_raw_parts(x_lp, len2)
	};
	let y = unsafe {
		slice::from_raw_parts(y, lag2)
	};
	...
	celt_pitch_xcorr_c(x_lp4.as_ptr(), y_lp4.as_ptr(), xcorr.as_mut_ptr(),
		len >> 2, max_pitch as c_int >> 2, arch);

	let best_pitch = find_best_pitch(&xcorr[0 .. max_pitch >> 2],
		&y_lp4, len4);
...
#[no_mangle]
pub extern fn remove_doubling(x :* const v16, max_period :c_int,
		min_period :c_int, n :c_int, t0_ :&mut c_int,
		prev_period :c_int, prev_gain :v16, _arch :c_int) -> v16 {
	static SECOND_CHECK :[u32; 16] = [0, 0, 3, 2, 3, 2, 5, 2, 3,
		2, 3, 2, 5, 2, 3, 2];
	...
	*t0_ /= 2;
	...
	let xb = unsafe {
		slice::from_raw_parts(x, (n + max_period) as usize)
	};
	...
	let (xx, mut xy) = dual_inner_prod_rs(
	xsl![0, n],
	xsl![0, n],
	xsl![(-t0), n]);
	...
	for k in 2u32 .. 16 {
		...
		let (xyp, xy2) = dual_inner_prod_rs(xsl![0, n],
			xsl![-t1, n], xsl![-t1b, n]);
		xy = half32!(xyp + xy2);
		...
		let mut thresh = max16!(qconst16!(0.3, 15),
	...
	let mut xcorr = [0.0; 3];
	for k in 0 .. 3 {
		xcorr[k] = inner_prod_rs(xsl![0, n],
			xsl![-((t + k) as isize - 1), n]);
	}
	let offset = if xcorr[2] - xcorr[0] > mul16_32_q15!(qconst16!(0.7, 15),
			xcorr[1] - xcorr[0]) {
		1
	} else if xcorr[0] - xcorr[2] > mul16_32_q15!(qconst16!(0.7, 15),
			xcorr[1] - xcorr[2]) {
		-1
	} else {
		0
...
#[no_mangle]
pub extern fn celt_pitch_xcorr_c(x :* const v16, y :* const v16,
		xcorr :* mut v32, len :c_int, max_pitch :c_int, arch :c_int) {
	...
	let xcorr = unsafe {
		slice::from_raw_parts_mut(xcorr, max_pitch)
	};
	let x = unsafe {
		slice::from_raw_parts(x, len)
	};
	...
	loop {
		...
		let mut sum = [0.0; 4];
		let yoff = unsafe { y.offset(i as isize) };
	...
	while i < max_pitch {
		let yoff = unsafe {
			slice::from_raw_parts(y.offset(i as isize), len)
		};
