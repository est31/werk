...
#[no_mangle]
pub extern fn celt_fir_c(x :* const v16, num :* const v16, y :* mut v16,
		n :c_int, ord :c_int, arch :c_int) {
	...
	for i4 in 0 .. (n - 3) / 4 {
		...
		unsafe {
			xcorr_kernel_rs(&rnum, x.offset((i - ord) as isize),
				&mut sum, ord, arch);
		}
	...
	for i in 4 * ((n - 3) / 4)  .. n {
		unsafe {
			let mut sum = sle!(*x.offset(i as isize));
			for j in 0 .. ord {
				sum = mac16_16!(sum, rnum[j],
...
#[no_mangle]
pub extern fn celt_iir(x :* const v32, den :* const v16, yp :* mut v16,
		n :c_int, ord :c_int, mem :* mut v16, arch :c_int) {
	let ord = ord as usize;
	let n = n as usize;
	...
	let den = unsafe {
		slice::from_raw_parts(den, ord)
	};
	let yp = unsafe {
		slice::from_raw_parts_mut(yp, n)
	};
	let mem = unsafe {
		slice::from_raw_parts_mut(mem, ord)
	};
	...
	for i4 in 0 .. (n - 3) / 4 {
		...
		unsafe {
			xcorr_kernel_rs(&rden, y.as_ptr().offset(i as isize),
				&mut sum, ord, arch);
		}
...
#[no_mangle]
pub extern fn _celt_autocorr(x :* const v16, ac :* mut v16,
		window :* const v16, overlap :c_int, lag :c_int, n :c_int,
		arch :c_int) -> c_int {
	let fast_n = n - lag;
	...
	let window = unsafe {
		slice::from_raw_parts(window, overlap)
	};
